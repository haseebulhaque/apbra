"""Exact APBRA136 admission fails closed on missing or invalid authority."""
import copy
...
spec = importlib.util.spec_from_file_location('bootstrap_orchestration', ROOT / 'scripts/check_bootstrap.py')
c = importlib.util.module_from_spec(spec)
spec.loader.exec_module(c)


class OrchestrationScopeTests(unittest.TestCase):
    def test_admission_and_negative_authority(self):
        task_path = 'tasks/APBRA-136-orchestration.json'
        ...
        for mutation in ['valid', *mutations, 'source-status', 'symlink']:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as directory:
                ...
                names.update(c.CAPSTONE_ORCHESTRATION_PATHS | {task_path})
                for name in names:
                    target = root / name
                ...
                if mutation is None:
                    ...
                    next(s for s in sources['sources'] if s['id'] == 'capstone-orchestration')['status'] = 'PROPOSED'
                    source_path.write_text(json.dumps(sources))
                elif mutation == 'symlink':
                    target = root / 'apps/web/src/execution.ts'
                    target.unlink()
                ...
                if mutation == 'valid':
                    ...
                    self.assertTrue(any('safe bootstrap scope: apps/web/src/execution.ts' in e for e in errors), errors)
