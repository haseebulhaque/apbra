"""Exact APBRA132 admission fails closed on missing or invalid authority."""
import copy
...
spec = importlib.util.spec_from_file_location('bootstrap_design', ROOT / 'scripts/check_bootstrap.py')
...
class DesignScopeTests(unittest.TestCase):
    def test_admission_and_negative_authority(self):
        task_path = 'tasks/APBRA-132-design-plan.json'
        ...
        for mutation in ['valid', *mutations, 'source-status', 'symlink']:
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as directory:
                ...
                names.update(c.CAPSTONE_DESIGN_PATHS | {task_path})
                for name in names:
                    target = root / name
                ...
                if mutation is None:
                    ...
                    next(s for s in sources['sources'] if s['id'] == 'capstone-design-plan')['status'] = 'PROPOSED'
                    source_path.write_text(json.dumps(sources))
                elif mutation == 'symlink':
                    target = root / 'apps/web/src/designPlan.ts'
                    target.unlink()
                ...
                if mutation == 'valid':
                    ...
                    self.assertTrue(any('safe bootstrap scope: apps/web/src/designPlan.ts' in e for e in errors), errors)
