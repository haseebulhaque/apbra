...
export const clarificationContextBinding=(value:unknown)=>stable(value);
const readinessBinding=(session:ClarificationSession)=>stable({sessionId:session.sessionId,contextBinding:session.contextBinding,originalRequest:session.originalRequest,mode:session.mode,limits:session.limits,rounds:session.rounds,corrections:session.corrections,designConflicts:session.designConflicts,currentInterpretation:session.currentInterpretation,unresolvedAmbiguities:session.unresolvedAmbiguities,confirmationSummary:session.confirmationSummary,lastAnalysis:session.analyses.at(-1)});
...
const clarificationCycles=(session:ClarificationSession)=>session.rounds.length+session.corrections.length+new Set(session.designConflicts.map(item=>item.detectedAt)).size;
const exhausted=(next:ClarificationSession)=>{next.state='HUMAN_REVIEW_REQUIRED';next.confirmedAt='';next.conflictReasons=[...next.conflictReasons,'Configured clarification-round limit exhausted while material ambiguity remains.'];return next};

export function createClarificationSession(input:{sessionId:string;mode:InteractionMode;originalRequest:string;limits:ClarificationLimits;contextBinding?:string}):ClarificationSession{
  ...
  const contextBinding=clean(input.contextBinding??clarificationContextBinding({originalRequest:input.originalRequest,mode:input.mode,limits:input.limits}));if(!contextBinding)throw new ClarificationStateError('A material input context binding is required.');
  return{schemaVersion:2,sessionId:input.sessionId,mode:input.mode,state:'REQUEST',originalRequest:input.originalRequest,limits:clone(input.limits),rounds:[],analyses:[],corrections:[],designConflicts:[],currentInterpretation:null,unresolvedAmbiguities:[],confirmationSummary:null,conflictReasons:[],confirmedAt:'',contextBinding,readinessBinding:''};
}
...
export function applyClarificationAnalysis(session:ClarificationSession,input:{state:Exclude<ClarificationState,'REQUEST'|'ANALYSING'|'CONFIRMED'>;interpretation:ConfirmedRequirementInterpretation;questions:IterativeClarificationQuestion[];unresolvedAmbiguities:string[];confirmationSummary:MaterialConfirmationSummary;conflictReasons:string[];analysedAt:string;knowledgeSources?:ClarificationKnowledgeSource[]}):ClarificationSession{
  ...
  if(input.state==='NEEDS_CLARIFICATION'){
    ...
    if(clarificationCycles(next)>=next.limits.maxRounds)return exhausted(next);
  ...
  next.state=input.state;next.readinessBinding=input.state==='READY_FOR_CONFIRMATION'?readinessBinding(next):'';return next;
}
...
export function correctClarificationUnderstanding(session:ClarificationSession,rawCorrection:string,correctedAt:string):ClarificationSession{
  if(session.state!=='READY_FOR_CONFIRMATION')throw new ClarificationStateError(`Correction is not accepted from ${session.state}.`);if(!rawCorrection.trim()||rawCorrection.length>session.limits.maxAnswerCharacters||!clean(correctedAt))throw new ClarificationStateError('A bounded correction and timestamp are required.');const next=clone(session),limitReached=clarificationCycles(next)>=next.limits.maxRounds;next.corrections.push({rawCorrection,correctedAt});next.readinessBinding='';if(limitReached)return exhausted(next);next.state='ANALYSING';next.confirmedAt='';return next;
}

export function confirmClarificationUnderstanding(session:ClarificationSession,input:{confirmedAt:string;readinessBinding:string;contextBinding:string}):ClarificationSession{
  const currentReadiness=readinessBinding(session);if(session.state!=='READY_FOR_CONFIRMATION'||!session.currentInterpretation||!session.confirmationSummary||!clean(input.confirmedAt)||session.schemaVersion!==2||!session.contextBinding||input.contextBinding!==session.contextBinding||!session.readinessBinding||input.readinessBinding!==session.readinessBinding||session.readinessBinding!==currentReadiness)throw new ClarificationStateError('Only the exact complete visible understanding and material input context can be confirmed.');const next=clone(session);next.state='CONFIRMED';next.confirmedAt=input.confirmedAt;return next;
}
...
export function reopenClarificationForDesignConflict(session:ClarificationSession,reasons:string[],detectedAt:string):ClarificationSession{
  const exactReasons=reasons.map(clean).filter(Boolean);if(session.state!=='CONFIRMED'||!clean(detectedAt)||!exactReasons.length||new Set(exactReasons).size!==exactReasons.length)throw new ClarificationStateError('Only a confirmed session with exact design-conflict evidence can re-enter analysis.');const next=clone(session),limitReached=clarificationCycles(next)>=next.limits.maxRounds;next.designConflicts.push(...exactReasons.map(reason=>({reason,detectedAt})));next.readinessBinding='';if(limitReached)return exhausted(next);next.state='ANALYSING';next.confirmedAt='';return next;
}
...
export function validateClarificationSession(session:ClarificationSession,requireConfirmed=false):ClarificationSession{
  ...
  if(![1,2].includes(session.schemaVersion)||!clean(session.sessionId)||!clean(session.originalRequest)||!['BUSINESS','ADVANCED'].includes(session.mode)||!validLimits(session.limits))reasons.push('Session envelope or configured limits are invalid.');
  if(session.schemaVersion===2){if(!clean(session.contextBinding??''))reasons.push('Session material input context binding is missing.');const bindingStates=['READY_FOR_CONFIRMATION','CONFIRMED'];if(bindingStates.includes(session.state)){if(!clean(session.readinessBinding??'')||session.readinessBinding!==readinessBinding(session))reasons.push('Session readiness is not bound to the exact visible interpretation and input context.')}else if(session.readinessBinding)reasons.push('Only ready or confirmed sessions may retain a readiness binding.');}
  const cycleCount=clarificationCycles(session);if(cycleCount>session.limits.maxRounds&&(session.state!=='HUMAN_REVIEW_REQUIRED'||cycleCount!==session.limits.maxRounds+1))reasons.push('Transcript exceeds the configured round limit.');
