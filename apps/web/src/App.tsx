...
export function App() {
  ...
  function editAnswer(id: string, value: string) { setSnapshot(null); setAnswers({...answers, [id]:value}); }
  return <main>
    ...
    <div className="layout"><section aria-labelledby="request-heading"><h2 id="request-heading">01 · Define the report</h2>
      ...
      <label htmlFor="prompt">Report request</label><textarea id="prompt" rows={7} maxLength={4000} value={prompt} onChange={e => {setPrompt(e.target.value); setSnapshot(null);}}/>
    ...
    </section><section aria-labelledby="progress-heading"><h2 id="progress-heading">Workflow progress</h2><p>Only deterministic golden-fixture requirements processing is available. Later stages have not run.</p>
      <ol className="stages">{unavailableStages().map(s => <li key={s.name}><span>{s.name}</span><strong>{s.name === 'RequirementsSnapshot' && snapshot ? 'CONFIRMED' : s.status}</strong></li>)}</ol>
    ...
    </section></div><KnowledgePanel/><footer>APBRA-130 · Local Capstone requirements snapshot</footer>
  </main>;
}
