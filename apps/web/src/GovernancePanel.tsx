...
export function GovernancePanel({candidate,onState}:{candidate:Candidate|null;onState:(state:string)=>void}){
 ...
 useEffect(()=>{epoch.current++;if(active.current)adapter.invalidate(active.current);active.current=null;setOperation(null);setRelease(null);setBusy(false);setError('');setEvidence(adapter.exportEvidence());},[candidate,route,adapter]);
 ...
 <button disabled={!candidate||busy} onClick={()=>void submit()}>Submit validated candidate to demo governance</button>
 <button className="secondary" disabled={!candidate||busy} onClick={()=>void submit(true)}>Test failed-candidate governance rejection</button>
 ...
 {operation.status==='PENDING_REVIEW'&&<><label htmlFor="review-reason">Demo reviewer decision reason</label><textarea id="review-reason" value={reason} maxLength={1000} onChange={e=>setReason(e.target.value)}/><button disabled={busy} onClick={()=>decide('APPROVE')}>Simulate independent reviewer approval</button><button disabled={busy} onClick={()=>decide('REQUEST_CHANGES')}>Simulate request changes</button><button disabled={busy} onClick={()=>decide('DECLINE')}>Simulate decline</button></>}
 {['CHANGES_REQUESTED','DECLINED','VALIDATION_FAILED'].includes(operation.status)&&<button disabled={!candidate||busy} onClick={()=>void resubmit()}>Resubmit preserved golden candidate</button>}
 <button disabled={busy||!['APPROVED','TRUSTED_ELIGIBLE'].includes(operation.status)} onClick={()=>void pack()}>Create demo release package</button></>}
