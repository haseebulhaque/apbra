...
it('uses stable heading-aware chunks with complete provenance and no overlap',()=>{
  const chunks=chunkDocuments();expect(chunks).toHaveLength(20);expect(new Set(chunks.map(c=>c.chunkId)).size).toBe(20);for(const chunk of chunks){expect(chunk.citation).toContain(chunk.source);expect(chunk.version).toBe('1.0.0');expect(chunk.text).toContain(chunk.heading);expect(chunk.tokenEstimate).toBeGreaterThan(5);expect(chunk.characterCount).toBe(chunk.text.length)}
});
it('performs exact cosine retrieval and retains citations',async()=>{
  const result=await retrieveKnowledge('accessible report contrast and keyboard order',3,embedder);expect(result.kind).toBe('SEMANTIC_LOCAL_RAG');expect(result.chunksIndexed).toBe(20);expect(result.retrieved).toHaveLength(3);expect(result.retrieved[0].source).toBe('accessibility-standards.md');expect(result.retrieved[0].citation).toContain('#AC-001');
});
