...
function azureAdapter(env:Record<string,string>):Plugin{
  ...
  async function handle(req:IncomingMessage,res:ServerResponse,next:()=>void){
    if(req.url==='/api/ai/status'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({configured,chatDeployment:chat,embeddingDeployment:embedding,apiMode:v1?'openai-v1':'deployment-api-version'}));return}
